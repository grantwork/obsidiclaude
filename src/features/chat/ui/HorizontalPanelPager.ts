...
export class HorizontalPanelPager<PanelId extends string> {
  ...
  #getTwoPanelReplica(panelId: PanelId): HorizontalPanelPagerPanel<PanelId> {
  ...
  #copyReplicaVisualState(sourceRoot: Element, replicaRoot: Element): void {
  ...
  #sanitizeReplicaTree(root: HTMLElement): void {
  ...
  #clearTwoPanelLayout(removeReplica = false): void {
    for (const element of this.twoPanelOrderedElements) {
      for (const orderClass of TWO_PANEL_ORDER_CLASSES) {
        element.classList.remove(orderClass);
      }
  ...
  #getNearestRenderedPanel(): HorizontalPanelPagerPanel<PanelId> | null {
  ...
  #announceDragTarget(direction: number): void {
  ...
  #getAdjacentPanel(direction: number): HorizontalPanelPagerPanel<PanelId> | null {
  ...
  #resolveActivePanel(requested: PanelId): PanelId | null {
