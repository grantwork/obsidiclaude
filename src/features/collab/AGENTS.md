...
- A committed Project selection change synchronously closes the old Project-management surface before notifying later subscribers. Modal reads and mutations remain bound to their captured Project and lifecycle generation, so an initial subscription callback or late completion cannot act on the replacement selection.
