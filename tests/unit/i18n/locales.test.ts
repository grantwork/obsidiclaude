...
describe('locale files', () => {
  ...
  it('uses commands-and-skills copy for hidden Claude entries', () => {
    ...
    expect(english['settings.hiddenSlashCommands.desc']).toBe(
      'Hide specific commands and skills from the dropdown. Useful for hiding Claude Code entries that are not relevant to Obclaudian. Enter names without the leading slash, one per line.',
    );
  });
