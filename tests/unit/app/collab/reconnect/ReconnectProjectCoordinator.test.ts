...
function cloudMembership(): CollabLocalCloudMembershipRecord {
  return {
    authority: {
      ...
      gitRemoteUrl: 'https://old.example.test/operator/v6/projects/project-a/repository.git',
      kind: 'cloud',
...
describe('ReconnectProjectCoordinator', () => {
  ...
  it('validates and durably relocates one exact Cloud Project binding', async () => {
    ...
    const cloudRelocation = {
      ...
      connect: jest.fn(async () => ({
        ...
        git: {
          headers: [],
          remoteUrl: 'http://new.example.test/proxy/cloud/v6/projects/project-a/repository.git',
        },
        projectId: 'project-a',
        readSnapshot,
    ...
    expect(currentMembership).toMatchObject({
      authority: {
        authorityGeneration: 4,
        gitRemoteUrl: 'http://new.example.test/proxy/cloud/v6/projects/project-a/repository.git',
        kind: 'cloud',
        serverUrl: 'http://new.example.test/proxy/cloud',
      },
  ...
  it('retains a prepared relocation and fails closed on an unexpected existing origin', async () => {
    currentMembership = cloudMembership();
    originUrls = ['https://unexpected.example.test/v6/projects/project-a/repository.git'];
    let pending: unknown = null;
    const activity = {
      activate: jest.fn(async () => undefined),
      resume: jest.fn(async () => undefined),
